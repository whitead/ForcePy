...
class CGUniverse(Universe):
    ...
    def lammps_data_string(self, fm, prefix='', folder = os.curdir, table_points=1000):
        ...
        #write data file
        with open('%s_fm.data' % prefix) as outout:
            ...
            for i,a in zip(range(len(self.atoms))):
                atom_section.append("%d %d %d %f %f %f %f %f\n" % (i+1, a.residue.id, 
                                                            a.charge, 
            ...
            bindex = 0
            for i,b in zip(range(len(self.bonds)),self.bonds):
                bond_section.append("%d %d %d %d\n" % (i+1, fm.get_bond_type(self.bonds[i].atom1, self.bonds[i].atom2),
                                                   self.bonds.atom1.id, self.bonds.atom2.id))
